...
export const Product3DViewer: React.FC<Product3DViewerProps> = ({
  ...
  // Determine cap color based on product slug if not provided
  const getCapColor = useCallback((): number => {
    if (capColor) return parseInt(capColor.replace('#', '0x'), 16);
    switch (product.slug) {
  ...
  useEffect(() => {
    ...
    // --- Bottle Geometry (Procedural Lathe Profile) ---
    ...
    // Add vertical ridges around the cap
    const numRidges = 24;
    ...
    // --- Label Cylinder & Texture Mapping ---
    const textureLoader = new THREE.TextureLoader();
    const imageSrc = product.image || '/images/products/bio-npk-consortia.jpg';
    ...
    textureLoader.load(
      ...
      (texture) => {
        ...
        const labelHeight = 2.05;
        const labelRadius = 1.006; // Slight offset to prevent z-fighting
        const labelGeo = new THREE.CylinderGeometry(labelRadius, labelRadius, labelHeight, 64, 1, true);

        // Map label texture seamlessly
        ...
        // Align front face of texture to camera
    ...
    const handlePointerMove = (e: PointerEvent) => {
      ...
      sceneRef.current.targetRotationX = Math.max(-0.4, Math.min(0.4, sceneRef.current.targetRotationX + deltaY * 0.008));
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      sceneRef.current.targetZoom = Math.max(4.0, Math.min(9.5, sceneRef.current.targetZoom + zoomDelta));
    };
  ...
  const handleZoomIn = () => {
    if (sceneRef.current) {
      sceneRef.current.targetZoom = Math.max(4.0, sceneRef.current.targetZoom - 1.2);
    }
  };

  const handleZoomOut = () => {
    if (sceneRef.current) {
      sceneRef.current.targetZoom = Math.min(9.5, sceneRef.current.targetZoom + 1.2);
    }
  };
  ...
  const handleReset = () => {
    if (sceneRef.current) {
      ...
      sceneRef.current.targetZoom = 6.5;
    }
  };
